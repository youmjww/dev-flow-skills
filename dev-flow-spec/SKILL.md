...
最初に以下のエージェントを起動（`team_name="doc-team"`, `name="doc-orchestrator"`, `run_in_background=true`, `model="claude-haiku-4-5-20251001"`）：
...
以下のプロンプトで Agent を起動（`team_name="doc-team"`, `name="test-spec-writer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`, `mode="acceptEdits"`）：
...
3b と同時に以下のプロンプトで Agent を起動（`team_name="doc-team"`, `name="api-spec-writer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`, `mode="acceptEdits"`）：
...
### 3d. モック HTML の生成（IS_GUI=true の場合）

3b・3c と同時に以下のプロンプトで Agent を起動（`team_name="doc-team"`, `name="mock-writer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`, `mode="acceptEdits"`）：
...
3b・3c・3d と同時に、以下のレビュアーエージェントを起動します。各レビュアーは writer からの SendMessage を待機します。

**test-spec-reviewer**（`team_name="doc-team"`, `name="test-spec-reviewer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`）：
...
**api-spec-reviewer**（IS_API=true の場合、`team_name="doc-team"`, `name="api-spec-reviewer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`）：
...
**mock-reviewer**（IS_GUI=true の場合、`team_name="doc-team"`, `name="mock-reviewer"`, `run_in_background=true`, `model="claude-sonnet-4-6"`）：
...
1. 各ドキュメントファイル（TEST_SPEC_PATH / API_SPEC_PATH / MOCK_PATH）の存在を Bash で確認する
...
例: `Agent(name="test-spec-writer", team_name="doc-team", run_in_background=true, model="claude-sonnet-4-6", mode="acceptEdits", prompt="以下の指摘を反映してテスト定義書を修正してください: {指摘内容}。完了後 test-spec-reviewer に報告してください。")`
