...
| `"blocked"` | `blocker_type` を確認して AskUserQuestion で人間に判断を仰ぐ |
...
同様に App Dev のシニアレビュアーエージェントを起動（`model="haiku"`, `disallowed_tools=["Edit","Write","NotebookEdit"]`）。
...
App QA のシニアレビュアーエージェントを起動（`model="haiku"`, `disallowed_tools=["Edit","Write","NotebookEdit"]`）。
テスト網羅性・独立性を確認。重大指摘があれば Sonnet に昇格。
