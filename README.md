...
flowchart TD
    ...
    P12["<b>Phase 1-2: 要件定義</b><br/>/dev-flow-requirements<br/>━━━━━━━━━<br/>・対話で要件を深掘り<br/>・曖昧表現リント・用語集整備<br/>・REQ-NNN ID を frontmatter に付与<br/><br/><i>モデル: Opus 4.7</i>"]

    P34["<b>Phase 3-4: ドキュメント生成</b><br/>/dev-flow-spec<br/>━━━━━━━━━<br/>・テスト/API/インフラ/モック並列生成<br/>・covers: [REQ-NNN] で要件を紐付け<br/>・OpenAPI 3.1.0 / Gherkin 形式<br/><br/><i>モデル: Haiku (子: Sonnet)</i>"]

    P44["<b>Phase 4.4: Impact Analysis</b><br/>incremental mode のみ<br/>━━━━━━━━━<br/>・baseline_commit 以降の差分分析<br/>・影響を受ける REQ/TC/API を特定<br/>・影響範囲のみチェックリスト化<br/><br/><i>モデル: Sonnet</i>"]

    P45["<b>Phase 4.5: 整合性チェック</b><br/>/dev-flow-consistency<br/>━━━━━━━━━<br/>・トレーサビリティID整合性<br/>・ドキュメント間の矛盾検出<br/>・カバレッジ行列を生成<br/>・Infra/App/Cross + DAG 分類<br/>・タスク分解・設計凍結<br/><br/><i>モデル: Haiku (子: Opus)</i>"]

    P5["<b>Phase 5: 並列実装</b><br/>/dev-flow-implementation<br/>━━━━━━━━━<br/>・DAG依存解決で並列実行<br/>・過去レビュー指摘を memory 注入<br/>・ファイルスコープガードレール<br/>・Implements:/Tests: フッター<br/>・推論トレースを記録<br/>・Plan Repair・独立レビュアー<br/><br/><i>モデル: Haiku → Sonnet</i>"]

    P6["<b>Phase 6: テスト実行</b><br/>/dev-flow-test<br/>━━━━━━━━━<br/>・Haiku で最大2回試行<br/>・2回失敗で Sonnet に自動昇格<br/>・解決不能なら人間にエスカレーション<br/><br/><i>モデル: Haiku → Sonnet</i>"]

    P78["<b>Phase 7-8: 準拠チェック</b><br/>/dev-flow-compliance<br/>━━━━━━━━━<br/>・カバレッジ行列で機械的検証<br/>・実装がドキュメントに準拠か確認<br/>・完了報告を生成<br/><br/><i>モデル: Opus 4.7</i>"]
